    ...
    <article className="app-card" onClick={onSelect} style={{ borderColor: isActive ? '#6366f1' : undefined }}>
      <header style={{ display: 'flex', gap: '0.8rem', alignItems: 'center' }}>
        {logoUrl ? (
          ...
          <div style={{ width: 36, height: 36, borderRadius: 8, background: 'var(--accent-gradient)', display: 'grid', placeItems: 'center' }}>👻</div>
        )}
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 600 }}>{application.company}</div>
          <div style={{ color: 'var(--text-secondary)', fontSize: 14 }}>{application.role}</div>
        </div>
        <span className="status-badge" style={{ background: `${statusColors[application.status]}22`, color: statusColors[application.status] }}>
          {application.status}
        </span>
      </header>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }} onClick={(e) => e.stopPropagation()}>
        ...
        <button type="button" className="secondary-btn" onClick={onDelete}>Delete</button>
      </div>
    </article>
  );
}
