...
export default function SmartTimingAdvisor({ application, ghostMeta }) {
  ...
  return (
    <aside style={cardStyle}>
      ...
      <p style={{ marginTop: 0, color: 'var(--text-secondary)' }}>{advice.reason}</p>
      <p>
        Recommended send time:{' '}
        <strong>{advice.sendAt.toLocaleString([], { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</strong>
      </p>
      <p style={{ marginBottom: 0 }}>
        Urgency:{' '}
        <strong style={{ color: urgencyColor[advice.urgency], textTransform: 'capitalize' }}>{advice.urgency}</strong>
      </p>
      <p style={{ marginBottom: 0, color: 'var(--text-secondary)' }}>{advice.strategy}</p>
    </aside>
  );
}
